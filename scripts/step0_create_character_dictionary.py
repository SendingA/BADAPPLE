...
def generate_novel(text):
    system_prompt = '''你是一个专业的小说作家。'''
    response = client.chat.completions.create(
        model="qwen3-30b-a3b",  # 或 "gpt-3.5-turbo"  "qwen-plus"
        messages=[
...
请按要求返回JSON格式响应，小说内容如下：{text}"""
    ...
    response = client.chat.completions.create(
        model="qwen3-30b-a3b",  # 或 "gpt-3.5-turbo"
        messages=[
...
def divide_image(text):
    prompt = f"""
    请将下面的一个场景描述，分割成1~3个画面(尽量少)，你的目标是将该场景通过若干静态画面来展现，去除掉其中的人物对话部分，注意严格遵循原场景的文字描述去生成画面，不要添加任何额外信息。
