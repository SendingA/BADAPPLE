    ...
    def get_tts_audio(self, message):
        ...
        for paragraph in tqdm.tqdm(message, desc="正在生成配音音频", unit="paragraphs"):
            ...
            for i, sentence in enumerate(paragraph):
                ...
                para_wavs.append(wav)
            wavs.append(np.concatenate(para_wavs))
...
def process_text_files(input_file, output_dir, language, gender):
    ...
    for scenario in scenarios:
        ...
        # Filter out empty parts and add period back
        sentences = [part + "。" for part in parts if part.strip()]
        # If the original text doesn't end with a period, remove it from the last sentence
        if not scenario.endswith("。") and sentences:
            sentences[-1] = sentences[-1][:-1]
        tasks.append(tuple(sentences))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gender', type=str, default="zm", help='声音的性别')
